...
module APIv2
  class Slanger < Grape::API
    desc 'Returns credentials used to subscribe to private Pusher channel. ' \
      'IMPORTANT: Pusher events are not part of Peatio public interface. ' \
      'The events may be changed or removed in further releases.'
    params do
      requires :channel_name, type: String, desc: 'The name of the channel being subscribed to.'
      requires :socket_id, type: String, desc: 'An unique identifier for the connected client.'
    end
    post '/slanger/auth' do
